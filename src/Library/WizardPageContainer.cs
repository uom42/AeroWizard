...
namespace AeroWizard
{
    ...
    public class WizardPageContainer : ContainerControl, ISupportInitialize
    {
        ...
        {
            get => field;
            set => SetCmdButtonText( BackButton , field = value );
        }
        ...
        public ButtonBase CancelButton
        {
            ...
            set
            {
                field?.Click -= cancelButton_Click;
                if ( value == null ) return;
        ...
        {
            get => field;
            set => SetCmdButtonText( CancelButton , field = value );
        }
        ...
        public virtual short PercentComplete
        {
            get
            {
                ...
                return pg == null
                    ...
                    : IsLastPage( pg )
                        ? ( short )100
                        : Convert.ToInt16( Math.Ceiling( (( double )Pages.IndexOf( SelectedPage ) + 1) * 100f / Pages.Count ) );
            }
        }
        ...
        {
            get => (selectedPage == null || Pages.Count == 0)
                ? null
                : selectedPage;
        ...
        internal WizardCommandButtonState GetCmdButtonState ( ButtonBase btn )
        {
            return btn?.Tag == null
                ...
                : btn.Tag is WizardCommandButtonState buttonState
                ? buttonState
                : btn.Enabled
                ? WizardCommandButtonState.Enabled
                : !btn.Visible ? WizardCommandButtonState.Hidden : WizardCommandButtonState.Disabled;
        }
        ...
        {
            if ( page == null || page.IsFinishPage ) return null;

            do
            {
                int pgIdx = Pages.IndexOf (page);
                page = page.NextPage ?? (pgIdx == Pages.Count - 1 ? null : Pages [pgIdx + 1]);
        ...
        private void InitialSetup ()
        {
            ...
            if ( selectedPage == null ) UpdateUIDependencies();
        ...
        private void Pages_AddItemHandler ( WizardPage item , bool selectAfterAdd )
        {
            ...
            item.Owner = this;
            item.Visible = false;
            if ( !Contains( item ) ) Controls.Add( item );

            if ( selectAfterAdd ) SelectedPage = item;
        }
        ...
        private void SetCmdButtonState ( ButtonBase btn , WizardCommandButtonState value )
        {
            ...
            switch ( value )
            {
                ...
                case WizardCommandButtonState.Hidden:
                    btn.Enabled = false;
                    if ( btn != BackButton ) btn.Visible = false;
